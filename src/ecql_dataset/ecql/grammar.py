...
# Порядок важен: составной оператор проверяется раньше простого, иначе
# `NOT CONTAINS` разберётся как `NOT` со значением `CONTAINS`.
...
def split_conditions(*, text: str) -> tuple[list[str], str | None]:
    ...
    Смешение `&&` и `||` - ошибка: приоритет операций языком не задан.
